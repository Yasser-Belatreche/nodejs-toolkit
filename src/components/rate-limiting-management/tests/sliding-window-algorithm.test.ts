...
import { SlidingWindowAlgorithm } from '../main/core/domain/algorithms/sliding-window-algorithm';

await describe('Sliding Window Algorithm', async () => {
    let userId: string;
    let token: string;
    const limits = defaultQuotaLimits;
    const algorithm = SlidingWindowAlgorithm.Instance(Library.aRedisClient());
    ...
    after(async () => {
        ...
        console.log(Library.aRedisClient().isOpen);
    });
